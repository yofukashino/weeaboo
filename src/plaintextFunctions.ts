import { OwO, SettingValues, UwU } from "@this";
...
export const _questionMySanity = (
  node: string | string[] | unknown,
  props?: Record<string, unknown>,
): string | string[] | unknown => {
  if (SettingValues.get("type") === "uwu") return uwuify(node, props);
  if (SettingValues.get("type") === "owo") return owoify(node, props);
  return node;
};
